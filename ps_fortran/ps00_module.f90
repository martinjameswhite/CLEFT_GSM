...
!     integer, parameter :: my_nmax = 30
     ...
     real(kind=dp), parameter :: Omega_M = 0.2950379
     real(kind=dp), parameter :: Omega_L = 0.7049621
...
! Two cases when n=1 and n>1
       if (nin==1) then
          ya(1:n) = rr(1:n)**(3.d0/2.d0)*fixedk**3*(VVin(1:n)+(1.d0/3.d0)&
                    *TTin(1:n))*exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))
       else
          ntemp1 = dble(nin) - 1.d0
          ntemp2 = 3.d0/2.d0 - ntemp1
          ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(fixedk**3&
                    *VVin(1:n)+fixedk**3*(1.d0/3.d0)*TTin(1:n)&
                   -(2.d0/3.d0)*ntemp1*fixedk*TTin(1:n)/YYin(1:n))&
...
! Two cases when n=0 and n>0
      if (nin==0) then
        ...
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(fixedk**2&
                  *XXin_eft_1(1:n)+fixedk**2*YYin_eft_1(1:n)&
                  -2.d0*ntemp1*YYin_eft_1(1:n)/YYin(1:n))&
                  *exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif
...
! Two cases when n=1 and n>1
       if (nin==1) then
          ya(1:n) = rr(1:n)**(3.d0/2.d0)*fixedk**3*VVin_eft_2(1:n)&
                    *exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))
       else
          ntemp1 = dble(nin) - 1.d0
          ntemp2 = 3.d0/2.d0 - ntemp1
          ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*fixedk**3&
                    *VVin_eft_2(1:n)*exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
       endif
...
! Two cases when n=1 and n>1
       if (nin==1) then
          ya(1:n) = rr(1:n)**(3.d0/2.d0)*fixedk**3*(VVin_eft_3(1:n)+(1.d0/3.d0)&
                    *TTin_eft_3(1:n))*exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))
       else
          ntemp1 = dble(nin) - 1.d0
          ntemp2 = 3.d0/2.d0 - ntemp1
          ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(fixedk**3&
                    *VVin_eft_3(1:n)+fixedk**3*(1.d0/3.d0)*TTin_eft_3(1:n)&
                   -(2.d0/3.d0)*ntemp1*fixedk*TTin_eft_3(1:n)/YYin(1:n))&
                   *exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
       endif
