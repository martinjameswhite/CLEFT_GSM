...
!==== My variables ====!
  ...
  real(kind=dp), dimension(n,27) :: in
...
! Read input data --- Note that data was prepared to fit the initialized rr(i) mash!
  ...
  open(20,file='../Data/Ens_DarkSky_4096_46_II_z00.dat',status='old')
  read(20,*)
  read(20,*)
  read(20,*)
    do j=1,n
...
!---     input-data    ---!
  ...
  sginloop = Dz**4.0_dp * in(:,5)/3.0_dp&
            +Dz**4.0_dp * in(:,6)/3.0_dp
! X0inloop = Dz**4.0_dp * 2.0_dp/3.0_dp * (        - in(:,7) - in(:,9))
!           +Dz**4.0_dp * 2.0_dp/3.0_dp * (        - in(:,8) - in(:,10))
  XXinloop = Dz**4.0_dp * 2.0_dp/3.0_dp * (in(:,5) - in(:,7) - in(:,9))&
            +Dz**4.0_dp * 2.0_dp/3.0_dp * (in(:,6) - in(:,8) - in(:,10))
  YYinloop = Dz**4.0_dp * 2.0_dp * in(:,9)&
            +Dz**4.0_dp * 2.0_dp * in(:,10)

  VVin = Dz**4.0_dp * 1.0_dp/5.0_dp* ( 2.0_dp*in(:,11) - 3.0_dp*in(:,12) )
  TTin = Dz**4.0_dp * 3.0_dp* in(:,12)

  Xiinlin = Dz**2.0_dp * in(:,13)
  Zetainloop = Dz**4.0_dp * in(:,14)
  Chiinloop = Dz**4.0_dp * in(:,15)

  U10inlin  = Dz**2.0_dp * in(:,16)
!  U10inloop = Dz**4.0_dp * in(:,17)
  U10insum  = Dz**2.0_dp * in(:,16) + Dz**4.0_dp * in(:,17)

  U11inloop  = Dz**4.0_dp * in(:,18)
  U20inloop  = Dz**4.0_dp * in(:,19)
  V10inloop  = Dz**4.0_dp * in(:,20)
  V12inloop  = Dz**4.0_dp * in(:,21)
  XX10inloop = Dz**4.0_dp * in(:,22)
  YY10inloop = Dz**4.0_dp * in(:,23)
  XX20inloop = Dz**4.0_dp * in(:,24)
  YY20inloop = Dz**4.0_dp * in(:,25)

  dXiinlin = Dz**4.0_dp * in(:,26)
  dU10inlin = Dz**4.0_dp * in(:,27)
...
!---  write into the file  ---!
    ...
    write (filename,"(A20,A3,A4)") "ps00_hh_DarkSky_46_z",zstr(nz),".dat"
...
!---  output spectra data ---!
    ...
    write(10,'(15ES20.10)') k_val, (P_dm_val(i), i=1,3), (P_bias_df_val(i), i=1,5),&
                            (P_bias_s2_val(i), i=1,4), (P_bias_hd_val(i), i=1,2)
