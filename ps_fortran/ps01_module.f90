...
subroutine ps01_dm(qqin,XXin,YYin,sgin,XXinloop,YYinloop,sginloop,VVin,TTin,fixedk,resPS)
...
!  call subrutines: Z, A, W
        ...
        call int01_Z(a,qqin,XXin,YYin,sgin,fixedk,nindex)
        ...
        call int01_A(a,qqin,XXin,YYin,sgin,XXinloop,YYinloop,sginloop,fixedk,nindex)
        call fht(n,a,dir,wsave)
        call nearest_interp_1d(n,kk,a,1,kout,numout)
        resPS(2) = resPS(2) - 8.0_dp*pi*sqrt(pi/2.d0)*kout(1)**(-3.d0/2.d0)*numout(1)

        if (nindex >= 1) then
        call int01_W(a,qqin,XXin,YYin,VVin,TTin,fixedk,nindex)
        call fht(n,a,dir,wsave)
        call nearest_interp_1d(n,kk,a,1,kout,numout)
        resPS(3) = resPS(3) + 8.0_dp*pi*sqrt(pi/2.d0)*kout(1)**(-3.d0/2.d0)*numout(1)
        endif

       enddo

end subroutine ps01_dm

!====================================================================================================================================!

subroutine ps01_bias(qqin,XXin,YYin,sgin,Xiinlin,U10inlin,U10inloop,U20inloop,U11inloop,XX10inloop,YY10inloop,fixedk,resPS)
...
!==== my variables ====!
      integer :: nindex,i
      real(kind=dp), dimension(n), intent(in) :: qqin,XXin,YYin,sgin
      real(kind=dp), dimension(n), intent(in) :: Xiinlin,U10inlin,U10inloop
...
!  call subrutines: Fp, Fp2, Fpp, FpFpp

        call int01_Fp(a,qqin,XXin,YYin,sgin,U10inlin,U10inloop,XX10inloop,YY10inloop,fixedk,nindex)
        call fht(n,a,dir,wsave)
        call nearest_interp_1d(n,kk,a,1,kout,numout)
        resPS(1) = resPS(1) + 4.0_dp*pi*sqrt(pi/2.d0)*kout(1)**(-3.d0/2.d0)*numout(1)

        call int01_Fp2(a,qqin,XXin,YYin,Xiinlin,U10inlin,U11inloop,fixedk,nindex)
        call fht(n,a,dir,wsave)
        call nearest_interp_1d(n,kk,a,1,kout,numout)
        resPS(2) = resPS(2) + 4.0_dp*pi*sqrt(pi/2.d0)*kout(1)**(-3.d0/2.d0)*numout(1)

        call int01_Fpp(a,qqin,XXin,YYin,U10inlin,U20inloop,fixedk,nindex)
        call fht(n,a,dir,wsave)
        call nearest_interp_1d(n,kk,a,1,kout,numout)
        resPS(3) = resPS(3) + 4.0_dp*pi*sqrt(pi/2.d0)*kout(1)**(-3.d0/2.d0)*numout(1)

!        if (nindex >= 1) then
        call int01_FpFpp(a,qqin,XXin,YYin,Xiinlin,U10inlin,fixedk,nindex)
        call fht(n,a,dir,wsave)
        call nearest_interp_1d(n,kk,a,1,kout,numout)
...
end subroutine ps01_bias

!====================================================================================================================================!
!            Integrands for dark matter
!====================================================================================================================================!

subroutine int01_Z(ya,rr,XXin,YYin,sgin,fixedk,nin)
...
! Two cases when n=0 and n>0
      if (nin==0) then
        ya(1:n) = rr(1:n)**(3.d0/2.d0)*(1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n))&
                  *exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))&
                  - fixedk**2*sgin(1:n)*exp(-fixedk**2*sgin(1:n)))
      else
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n))&
                  -ntemp1)*exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif


end subroutine int01_Z

!============================================

subroutine int01_A(ya,rr,XXin,YYin,sgin,XXinloop,YYinloop,sginloop,fixedk,nin)
...
! Two cases when n=0 and n>0
      if (nin==0) then
        ya(1:n) = rr(1:n)**(3.d0/2.d0)*(1.d0/2.d0*fixedk**2*(XXinloop(1:n)+YYinloop(1:n))&
                  *exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))&
                  - fixedk**2*sginloop(1:n)*exp(-fixedk**2*sgin(1:n)))
      else
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(1.d0/2.d0*fixedk**2*(XXinloop(1:n)+YYinloop(1:n))&
                  - ntemp1*YYinloop(1:n)/YYin(1:n))*exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif

end subroutine int01_A

!============================================

subroutine int01_W(ya,rr,XXin,YYin,VVin,TTin,fixedk,nin)
...
! Two cases when n=1 and n>1
       if (nin==1) then
          ya(1:n) = rr(1:n)**(3.d0/2.d0)*1.d0/2.d0*fixedk**3*(VVin(1:n)+(1.d0/3.d0)*TTin(1:n))*exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))
       else
          ntemp1 = dble(nin) - 1.d0
          ntemp2 = 3.d0/2.d0 - ntemp1
          ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(1.d0/2.d0*fixedk**3*(VVin(1:n)+(1.d0/3.d0)*TTin(1:n))&
                   -(1.d0/3.d0)*ntemp1*fixedk*TTin(1:n)/YYin(1:n))*exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
       endif

end subroutine int01_W

!====================================================================================================================================!
!            Integrands for biasing
!====================================================================================================================================!

subroutine int01_Fp(ya,rr,XXin,YYin,sgin,U10inlin,U10inloop,XX10inloop,YY10inloop,fixedk,nin)
      ...
      real(kind=dp), intent(in) :: U10inlin(1:n),U10inloop(1:n)
      real(kind=dp), intent(out) :: ya(1:n)
      real(kind=dp) :: ntemp1,ntemp2
...
! Two cases when n=0 and n>0
      if (nin==0) then
        ya(1:n) = rr(1:n)**(3.d0/2.d0)*(-3.d0/2.d0*fixedk**2*(XX10inloop(1:n)+YY10inloop(1:n))&
                  *exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))&
                  + 3.d0*fixedk**2*sig10(1:n)*exp(-fixedk**2*sgin(1:n)))
      else
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(&
                  - rr(1:n)*(U10inlin(1:n)+3.d0*U10inloop(1:n))/YYin(1:n)&
                  - 3.d0/2.d0*fixedk**2*(XX10inloop(1:n)+YY10inloop(1:n))&
                  + rr(1:n)*U10inlin(1:n)*fixedk**2*(XXin(1:n)+YYin(1:n))/YYin(1:n)&
                  + (3.d0*ntemp1*YY10inloop(1:n)-2.d0*(ntemp1-1.d0)*rr(1:n)*U10inlin(1:n))/YYin(1:n))&
                  *exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif

end subroutine int01_Fp

!====================================================================================================================================!

subroutine int01_Fp2(ya,rr,XXin,YYin,Xiinlin,U10inlin,U11inloop,fixedk,nin)
...
! Two cases when n=0 and n>0
      if (nin==0) then
        ya(1:n) = rr(1:n)**(3.d0/2.d0)*fixedk**2*(-1.d0/2.d0*Xiinlin(1:n)*(XXin(1:n)+YYin(1:n)) - (U10inlin(1:n))**2)&
                  *exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))
      else
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(&
                    - rr(1:n)*U11inloop(1:n)/YYin(1:n)&
                    - 1.d0/2.d0*fixedk**2*Xiinlin(1:n)*(XXin(1:n)+YYin(1:n))& 
                    + ntemp1*Xiinlin(1:n) - fixedk**2*U10inlin(1:n)**2&
                    + 2.d0*ntemp1*U10inlin(1:n)**2/YYin(1:n))&
                    *exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif

end subroutine int01_Fp2

!====================================================================================================================================!

subroutine int01_Fpp(ya,rr,XXin,YYin,U10inlin,U20inloop,fixedk,nin)
...
! Two cases when n=0 and n>0
      if (nin==0) then
        ya(1:n) = rr(1:n)**(3.d0/2.d0)*(-1.d0)*fixedk**2*U10inlin(1:n)**2*exp(-1.d0/2.d0*fixedk**2*(XXin(1:n)+YYin(1:n)))
      else
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(&
                 - rr(1:n)*U20inloop(1:n)/YYin(1:n)&
                 - fixedk**2*U10inlin(1:n)**2&
                 + 2.d0*ntemp1*U10inlin(1:n)**2/YYin(1:n))&
                 *exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif

end subroutine int01_Fpp

!====================================================================================================================================!

subroutine int01_FpFpp(ya,rr,XXin,YYin,Xiinlin,U10inlin,fixedk,nin)
...
! Two cases when n>=1
      ...
      if (nin==0) then
        ...
        ntemp1 = dble(nin)
        ntemp2 = 3.d0/2.d0 - ntemp1
        ya(1:n) = rr(1:n)**ntemp2*(fixedk*YYin(1:n))**ntemp1*(-Xiinlin(1:n)*rr(1:n)*U10inlin(1:n)/YYin(1:n))&
                  *exp(-1.d0/2.d0*fixedk**2.d0*(XXin(1:n)+YYin(1:n)))
      endif

end subroutine int01_FpFpp
