...
def _template0(k, func, extrap = False):
...
def _template(k, func, extrap = False):
...
def _integrate(func, pool, taylor = 0):
    '''Do the \mu integrals for all j's by calling the templates
    '''

    p0  = pool.starmap(_template0, zip(kv, repeat(func)))
    
    pl  = pool.starmap(_template, zip(kv, repeat(func)))
...
def make_table(cl, kmin = 1e-3, kmax = 3, nk = 100, npool = 2, z = 0, M = 0.3):
 ...
 P_dd^2    P_s^2    P_ds^2    P_d^2s^2   P_s^2s^2   P_D2d     P_dD2d"
    ...
    pktable[:, 0] = kv[:]        
    pktable[:, 1] = _integrate(func = _za, pool = pool)
    pktable[:, 2] = _integrate(_aloop, taylor = 2, pool = pool)
    pktable[:, 3] = _integrate(_wloop, taylor = 3, pool = pool)
    pktable[:, 4] = _integrate(_b1, pool = pool)
    pktable[:, 5] = _integrate(_b1sq, pool = pool)
    pktable[:, 6] = _integrate(_b2, pool = pool)
    pktable[:, 7] = _integrate(_b2sq, pool = pool)
    pktable[:, 8] = _integrate(_b1b2, pool = pool)
    pktable[:, 9] = _integrate(_bs2, pool = pool)
    pktable[:,10] = _integrate(_b1bs2, pool = pool)
    pktable[:,11] = _integrate(_b2bs2, pool = pool)
    pktable[:,12] = _integrate(_bs2sq, pool = pool)
...
###################################################################################
def save_qkernel(cl, qfile):
    '''Save q functions from the CLEFT object 'cl' into the file 'qfile'
    '''
...
def save_rkernel(cl, rfile):
    '''Save r functions from the CLEFT object 'cl' into the file 'rfile'
    '''
    header = 'k[h/Mpc]   R1   R2 \n'
    np.savetxt(rfile, np.array([cl.qf.kr, cl.qf.R1, cl.qf.R2]).T, fmt='%0.4e', header=header)

def save_qfunc(cl, qfuncfile):
    cl.qf.save_qfunc(qfuncfile)
